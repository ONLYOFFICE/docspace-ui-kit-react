 ...
 *
 * The packaged ai-chat does declare the field (0.5.108-docspace.1), so this is
 * no longer standing in for a missing type — it validates. The questions are
 * generated by a model and travel through `attachments/save-files-many`, and a
 * half-formed entry would render a chip with an empty label or send an empty
 * prompt, so each one is checked before it is shown.
 */
