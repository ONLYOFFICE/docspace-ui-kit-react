...
// → prefix: "ui-interactive-elements-textarea"
const STORY_BASE = "ui-interactive-elements-textarea";
...
test.describe("Textarea — light", () => {
  ...
  test("states", async ({ page }) => {
    ...
    await expect(page).toHaveScreenshot("textarea-states.png");
  });
...
test.describe("Textarea — dark", () => {
  ...
  test("states dark", async ({ page }) => {
    await gotoStory(page, "states");
    await page.evaluate(() => document.body.classList.add("dark"));
    await expect(page).toHaveScreenshot("textarea-states-dark.png");
  });
