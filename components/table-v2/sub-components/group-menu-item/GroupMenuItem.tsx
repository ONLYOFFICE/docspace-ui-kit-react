...
export const GroupMenuItem = memo(function GroupMenuItem({
  ...
  return (
    ...
    >
      <Button
        id={id}
        className={classNames(styles.groupMenuItemButton, {
      ...
      {withDropDown && (
        <DropDown
          ...
          maxHeight={
            fixedDropdownStyles
              ? isTabletOrBelow
                ? Math.min(options?.length ?? 5, 5) * ITEM_HEIGHT_TABLET_MOBILE
        ...
        >
          {options?.map((option) => {
            const { key, ...rest } = option as Record<string, unknown>;
            return (
              <DropDownItem
                key={key as string}
                {...rest}
