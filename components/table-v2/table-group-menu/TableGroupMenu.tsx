...
const GroupMenuBody = memo(
  ({ headerMenu, isBlocked, isMobileView }: GroupMenuBodyProps) => (
    <Scrollbar className={styles.groupMenuScrollBar}>
      {headerMenu.map((item) => (
        <GroupMenuItem
...
export const TableGroupMenu = memo((props: TableGroupMenuProps) => {
  ...
  return (
    ...
    >
      ...
      {!withoutInfoPanelToggler && (
        ...
        >
          <div
            className={classNames(styles.infoPanelToggleBg, "info-panel-toggle-bg")}
          >
            <IconButton
