...
`tabIndex={0}` is not decoration — without it the field is not reachable by keyboard.
...
_Generated by `pnpm readme:props` from `TextInput.types.ts`. Do not edit; edit the JSDoc._
...
| `forwardedRef`      | `React.Ref<HTMLInputElement>`                                                         | no       | –                | Forwarded ref. Applied to the plain input only — when `mask` is set the masked input is rendered instead and the ref is dropped.                                   |
| `guide`             | `boolean`                                                                             | no       | `false`          | When guide is true, Text Mask always shows both placeholder characters and non-placeholder mask characters                                                         |
...
| `inputMode`         | `"none" \| "text" \| "decimal" \| "numeric" \| "tel" \| "search" \| "email" \| "url"` | no       | –                | Input mode for virtual keyboard                                                                                                                                    |
| `isAutoFocussed`    | `boolean`                                                                             | no       | `false`          | Focus the input field on initial render                                                                                                                            |
...
| `mask`              | `Mask \| ((value: string) => Mask)`                                                   | no       | –                | Input text mask                                                                                                                                                    |
...
| `testId`            | `string`                                                                              | no       | `"text-input"`   | HTML data-testid attribute                                                                                                                                         |
| `withBorder`        | `boolean`                                                                             | no       | `true`           | Indicates that component contains border                                                                                                                           |

Plus the rest of `React.InputHTMLAttributes<HTMLInputElement>`, forwarded to the element —
except `size`, `type`, `value`, `onChange`, `onBlur`, `onFocus`, `onKeyDown`, `onClick`,
`onContextMenu`, `disabled`, `readOnly`, `className`, `style`, `dir` and `ref`, which the
props above replace.
<!-- props:end -->
...
- **`tabIndex` defaults to `-1`, so the field is not in the tab order.** Every field a user
  is supposed to reach with the keyboard needs an explicit `tabIndex={0}`. Nothing warns,
...
- **Not focusable by default** — see `tabIndex` above. This is the single most likely
  accessibility defect in a form built from this component.
