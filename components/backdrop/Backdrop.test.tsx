...
describe("<Backdrop />", () => {
  ...
  describe("shouldShowBackdrop prop", () => {
    it("forces backdrop to show when shouldShowBackdrop is true", () => {
      // First render a backdrop to create existing backdrop
      render(<Backdrop visible />);

      // Second backdrop with shouldShowBackdrop should also render
      render(<Backdrop visible shouldShowBackdrop />);

      const backdrops = screen.queryAllByTestId("backdrop");
      expect(backdrops).toHaveLength(2);
    });

    it("respects shouldShowBackdrop even with multiple existing backdrops", () => {
      render(<Backdrop visible isAside />);
      render(<Backdrop visible isAside />);
      render(<Backdrop visible shouldShowBackdrop />);
  ...
  describe("z-index behavior", () => {
    it("applies default z-index of 203", () => {
      render(<Backdrop visible />);
      expect(screen.getByTestId("backdrop")).toHaveStyle({ zIndex: 203 });
    });

    it("applies custom z-index", () => {
      render(<Backdrop visible zIndex={500} />);
      expect(screen.getByTestId("backdrop")).toHaveStyle({ zIndex: 500 });
    });

    it("applies z-index of 0", () => {
      render(<Backdrop visible zIndex={0} />);
      expect(screen.getByTestId("backdrop")).toHaveStyle({ zIndex: 0 });
    });
  });
