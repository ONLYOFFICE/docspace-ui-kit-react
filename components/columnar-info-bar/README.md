...
| `closeLabel` | `string`                           | no       | `"Close"`   | Accessible name of the close button (its `aria-label`). Defaults to the English `"Close"`; pass a translated string. Ignored without `onAction`.                                       |
| `headerText` | `string`                           | no       | –           | Bold caption above the columns, rendered as an `<h3>` and omitted entirely when empty.                                                                                                 |
