...
| `labelFor`          | `string`        | no       | –                   | `id` of the control this labels, which becomes the label's `for`. Give the control that same `id` and the caption becomes clickable and is announced with the field; without it the label captions nothing. |
