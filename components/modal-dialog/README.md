...
| `closeOnBackdropClick`   | `boolean`                                                                                                                      | no       | `true`                  | Whether a click on the backdrop closes the dialog. Unlike `isCloseable`, this stops only that one route; the cross and Escape keep working.                                                                           |
| `containerVisible`       | `boolean`                                                                                                                      | no       | `false`                 | **`ASIDE-ONLY`** Allows embedding modal as aside dialog inside parent container                                                                                                                                       |
| `dataTestId`             | `string`                                                                                                                       | no       | `"aside-header"`        | Test id                                                                                                                                                                                                               |
...
| `isCloseable`            | `boolean`                                                                                                                      | no       | `true`                  | Whether the user may close the dialog at all. Every route to `onClose` — the header's cross, Escape, a backdrop click — runs through one guard, so `false` stops all three and leaves closing entirely to the caller. |
...
| `withoutBorder`   | `boolean`      | no       | –       | Hides the bottom border, which otherwise spans the full width of the panel regardless of the header's own side margins.                                                                   |
...
- **The dialog cannot be labelled through props.** Nothing links the header text to the
  element carrying the role, and while `aria-label` type-checks — the props type is open —
  unknown props are forwarded to the internal `AsideHeader`, not to that element, and only
  when a header is rendered at all. So an `aria-label` you pass lands on the header or
  nowhere. Giving the dialog an accessible name needs a fix in the component.
