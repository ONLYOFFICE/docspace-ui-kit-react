...
- **Reading storage during render breaks server rendering.** There is no `localStorage` on the
  server, so the component throws unless it is rendered on the client only.
