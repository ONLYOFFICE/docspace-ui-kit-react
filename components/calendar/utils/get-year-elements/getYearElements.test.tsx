...
describe("getYearElements", () => {
  beforeEach(() => {
    ...
    vi.mocked(dateUtils.now).mockReturnValue(
      DateTime.fromObject({ year: 2023, month: 1, day: 1 }),
    );
  });
  // 2023-01-01
  ...
  it("should render year elements", () => {
    const elements = getYearElements(
      years,
      setObservedDate,
      setSelectedScene,
      mockSelectedDate,
  ...
  it("should handle year clicks for different types", () => {
    vi.mocked(dateUtils.now).mockReturnValue(
      DateTime.fromObject({ year: 2024, month: 5, day: 10 }),
    );
