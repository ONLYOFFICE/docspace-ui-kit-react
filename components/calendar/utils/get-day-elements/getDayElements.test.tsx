...
describe("getDayElements", () => {
  const mockObservedDate = DateTime.fromObject({
    year: 2023,
    month: 10,
    day: 1,
  });
  const mockSelectedDate = DateTime.fromObject({
    year: 2023,
    month: 10,
    day: 15,
  });
  ...
  it("should handle date clicks for all day types", () => {
    ...
    expect(handleDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ day: 29, month: 9 }),
    );
    handleDateChange.mockClear();
    ...
    expect(handleDateChange).toHaveBeenCalledWith(
      expect.objectContaining({ day: 2, month: 11 }),
    );
    handleDateChange.mockClear();

    const day15 = screen
      .getAllByText("15")
      .find((el) => !el.closest("button")?.hasAttribute("disabled"));
    if (day15) {
      fireEvent.click(day15);
      expect(handleDateChange).toHaveBeenCalledWith(
        expect.objectContaining({ day: 15, month: 10 }),
      );
      handleDateChange.mockClear();
    }
  });
  ...
  it("should mark disabled dates", () => {
    ...
    const restrictiveMin = DateTime.fromObject({
      year: 2023,
      month: 10,
      day: 10,
    });
    const restrictiveMax = DateTime.fromObject({
      year: 2023,
      month: 10,
      day: 20,
    });
