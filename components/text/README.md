...
- **Three props are dead.** `display`, `containerWidth` and `containerMinWidth` are declared,
  nothing reads them, and each reaches the DOM as an unknown attribute. `display` in particular
  does not set the CSS property its name suggests — `isInline` and `style` are the ways to do
  that.
