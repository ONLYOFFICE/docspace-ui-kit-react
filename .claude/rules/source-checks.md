...
## License headers -- this package has none, deliberately

**Do not add a license header to files here.** This package is **MIT**, and it declares that
the way `@onlyoffice/ai-chat` does: once in `package.json`, once in `LICENSE`, once in the
README, and not in individual source files. The 1 285 AGPL headers that used to be here were
removed with `scripts/relicense-mit.mjs`.

The monorepo's license-header check does **not** apply to this package and never did:
`getSourceFiles()` in `common/tests/utils/license.js` calls
`getWorkSpaces({ excludeUiKit: true })`. The client-side `source-checks.md` claims
`libs/ui-kit` is in scope "for all of them", which is inaccurate for this one check -- verified
by running `npm --prefix ./common/tests run test:license` with every header removed: it passes.
